...
- Directional maximum utilization and explicit down/stale/unknown states. Parallel connections use wider, stable curves so their individual utilization labels remain readable. The topology is fetched every 60 seconds; link states are re-evaluated every 10 seconds from the current snapshot. Neither refresh relayouts the graph.
